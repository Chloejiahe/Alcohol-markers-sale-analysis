...
# 1. 准备博弈矩阵数据
# 我们需要找到 Top 10 规格中，不同单只价格点的分布情况
matrix_df = biz_df[biz_df['支数'].isin(top_10_specs)].copy()

# 将支数转为字符串，确保 X 轴作为分类变量展示更清晰
matrix_df['规格(支)'] = matrix_df['支数'].astype(str) + "支"

# 聚合数据：按 规格 和 单只价格区间 计算总销量和平均单价
# 这样可以观察在同一个规格下，不同定价策略的表现
bubble_data = matrix_df.groupby(['规格(支)', '单只价格区间'], observed=False).agg({
    '销量': 'sum',
    '单只价格': 'mean'
}).reset_index()

# 剔除销量为 0 的静默区间，让图表更聚焦
bubble_data = bubble_data[bubble_data['销量'] > 0]
...
fig_matrix = px.scatter(
    ...
    color='单只价格区间',
    hover_name='单只价格区间',
    size_max=60,  # 调整气泡最大尺寸，使其更具视觉冲击力
    category_orders={"规格(支)": [f"{s}支" for s in sorted(top_10_specs)]},
    title="核心规格 vs 单只定价：市场博弈矩阵",
    labels={'单只价格': '平均单只价格 (元)', '规格(支)': '产品规格'},
...
fig_matrix.update_layout(
    xaxis_title="产品规格 (Top 10)",
    yaxis_title="平均单只价格 (元)",
    legend_title="定价区间",
    height=600,
    hovermode='closest'
)

# 增加辅助线：展示行业公认的“大众平价”基准线 (假设为 0.5元)
fig_matrix.add_hline(y=0.5, line_dash="dot", line_color="red", 
                  annotation_text="大众平价分水岭 (0.5元)", 
...
with col1:
    main_spec = spec_total.iloc[0]['支数']
    st.metric("核心主战场", f"{main_spec} 支装", "市场容量最大")
    st.caption(f"数据显示 {main_spec} 支规格在各大价位段均有布局，是入局必争之地。")

with col2:
    # 计算销量最大的单价区间
    top_price_interval = biz_df.groupby('单只价格区间', observed=False)['销量'].sum().idxmax()
    st.metric("黄金定价带", top_price_interval.split(' ')[1], "最易转化")
    st.caption("该价格区间贡献了全店/全行业最高的成交频次。")

with col3:
    # 找出单价高且有销量的规格（代表蓝海或高端机会）
    premium_potential = bubble_data[bubble_data['单只价格'] > 2].sort_values('销量', ascending=False)
    if not premium_potential.empty:
        opp_spec = premium_potential.iloc[0]['规格(支)']
        st.metric("高端机会点", opp_spec, "高溢价空间")
        st.caption(f"在 {opp_spec} 规格下，高单价产品依然存活，暗示品牌溢价机会。")
    else:
        st.metric("高端机会点", "待挖掘", "暂无显著数据")
